...
x1 = torch.tensor(x1, dtype=torch.float32).to(device)  # float32
len1 = x.shape[0]
...
len5 = x1.shape[0]
x1 = torch.reshape(x1,(len5,16384))
len2 = x_test.shape[0]
...
net_f.load_state_dict(torch.load('finlf0.01.pkl'))
net_c.load_state_dict(torch.load('finlc0.01.pkl'))
net_f1.load_state_dict(torch.load('finlf10.01.pkl'))
net_c1.load_state_dict(torch.load('finlc10.01.pkl'))
...
for epoch in range(epochs):
    for batch_idx in tqdm(range(len(train)), desc='train'):
        ...
        if data.size()[0] == predict_the.size()[0]:
            ...
            #测试集
            predict_5 = net_f(x1).requires_grad_(True).to(device)
            predict_6 = net_c(x1).requires_grad_(True).to(device)
            u = predict_5
            o = predict_6
            samples = normal.sample(u.shape).to(device) 
            resampled = (o * samples + u).to(device)
            predict_7 = net_f1(resampled).requires_grad_(True).to(device)
            predict_8 = net_c1(resampled).requires_grad_(True).to(device)            
            u = predict_7
            o = predict_8
            x = x1
            o = torch.clamp(o, epsilon)  # 限制最小值
            p = (1 / (torch.sqrt(torch.tensor(2 * math.pi)) * o))
            y = -((x - u) ** 2) / (2 * o ** 2)
            p_3 = -torch.log(p * torch.exp(y) + epsilon)
            
            loss_c = torch.mean(p_1)+ (torch.mean(p_2)+the2*torch.mean(p_3))*the
    ...
    for batch_idx in tqdm(range(len(test)), desc='test'):
        ...
        if test_data.size()[0] == predict_the.size()[0]:
            ...
            predict_1 = net_f(x1).requires_grad_(True).to(device)
            predict_2 = net_c(x1).requires_grad_(True).to(device)
            #reshample
            samples = normal.sample(predict_1.shape).to(device)  # 从标准正态分布中抽样，得到与mean相同形状的样本
            resampled = (predict_2 * samples + predict_1).to(device)            
            #磁场
            predict_3 = net_f1(resampled).requires_grad_(True).to(device)
            predict_4 = net_c1(resampled).requires_grad_(True).to(device)
            u = predict_3
